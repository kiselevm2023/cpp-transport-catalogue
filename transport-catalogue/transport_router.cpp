...
const double SPEED_CONVERSION_COEFF = MINUTES_PER_HOUR / METERS_PER_KM;
...
TransportRouter::TransportRouter(const TransportCatalogue& catalogue,
    ...
    BuildRoute(); // сразу строим маршруты
}
...
double TransportRouter::DistanceIntoTime(double distance) const {
    return (distance * SPEED_CONVERSION_COEFF) / settings_.bus_velocity;
}
...
const std::pair<graph::VertexId, graph::VertexId>& TransportRouter::GetVertexFromStop(const Stop* stop) const {
    ...
    if (vertex == stop_to_vertex_.end()) {
        throw std::runtime_error("Stop not found in stop_to_vertex_");
    }
...
void TransportRouter::AddBusIntoGraph(const Bus& bus) {
    for (std::size_t i = 0; i < (bus.stops.size() - 1); ++i) {
        ...
        for (std::size_t j = i; j < (bus.stops.size() - 1); ++j) {
            ...
            AddBusEdgeIntoGraph(i_from, to, bus.name, static_cast<int>(j + 1 - i), from_to_distance);

            if (!bus.is_roundtrip) {
                to_from_distance += catalogue_.GetDistanceBetweenStops(to, from);
                AddBusEdgeIntoGraph(to, i_from, bus.name, static_cast<int>(j + 1 - i), to_from_distance);
            }
        }
    }
}
...
// Глобальная функция для перестроения маршрутов
namespace transport_router {
void BuildRoute(TransportRouter& router) {
    router.BuildRoute();
}
} // namespace transport_router